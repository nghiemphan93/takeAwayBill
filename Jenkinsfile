        ...
        stage('build') {
            steps {
                script {
                    ...
                    echo "$DOCKER_PASS"
                    sh "${dockerLoginCMD}"
                    sh "docker build -t docker.nghiemphan.de/admin/takeAwayBill-backend:latest ./backend/Dockerfile"
//                    sh "docker push https://docker.nghiemphan.de/takeAwayBill-backend"
                }
            }
        }
        stage('deploy') {
